...
const initTimelime = () => new mojs.Timeline();

const useClapAnimation = (btnRef, countRef, totalRef) => {
  const [animationTimeline, setAnimationTimeline] = useState(initTimelime());
...
const ClapCount = ({ style: userStyles = {}, className }) => {
  ...
  return (
    <span ref={countRef} className={classNames} style={userStyles}>
      +{count}
    </span>
  );
};
...
const CountTotal = ({ style: userStyles = {}, className }) => {
  ...
  return (
    <span ref={totalRef} className={classNames} style={userStyles}>
      {countTotal}
    </span>
  );
};
